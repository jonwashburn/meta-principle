...
\paragraph{Formal verification.} Core structural theorems (meta--principle, atomic tick, conservation, $\delta$--potential uniqueness, cost uniqueness, and eight--tick minimality) are machine--checked in Lean~4 (file \texttt{IndisputableMonolith.lean}). These results support the structural assumptions invoked here while we present the phenomenology in standard classical notation.
...
2. \emph{Laboratory short--range gravity}: Within the $10$--$100$\,$\mu$m band, the model predicts \emph{no} resolvable deviation from Newtonian gravity at current sensitivities (consistent with recent micromechanical constraints, e.g., \cite{lee2020}). At nanometre scales, present techniques do not yet permit clean gravitational measurements; RS--motivated running suggests at most an $\mathcal{O}(1)$ variation around $\sim 20$\,nm, which we regard as prospective and not a near--term falsifier. We therefore adopt a laboratory null in this work and focus on galactic/cosmological tests.
